...
<div class="page clearfix">
	<form class="form-horizontal form form-validate" id="form1" action="" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<p class="form-control-static">
					门店新客立减<?php  echo $activity['data']['back'];?>元, 平台承担: <?php  echo $activity['data']['plateform_charge'];?>元, 代理商承担: <?php  echo $activity['data']['agent_charge'];?>元, 商户承担: <?php  echo $activity['data']['store_charge'];?>元
				</p>
			</div>
		</div>
		<?php  } else { ?>
				...
				<div class="col-sm-8">
					<div class="input-group">
						...
						<?php  if(!empty($_W['ismanager'])) { ?>
							...
							<input type="text" name="agent_charge" value="<?php  echo $activity['data']['agent_charge'];?>" class="form-control">
							<span class="input-group-addon">元</span>
		...
		<div class="form-group">
			<div class="col-sm-9 col-xs-9 col-md-9">
				...
				<?php  if(!empty($activity['id'])) { ?>
					<?php  if(empty($activity['status'])) { ?>
						<a href="<?php  echo iurl('store/activity/newMember/del');?>" class="btn btn-danger js-post" data-confirm="确定删除此活动?">删除</a>
					<?php  } else { ?>
						<a href="<?php  echo iurl('store/activity/newMember/del');?>" class="btn btn-danger js-post" data-confirm="确定撤销此活动?">撤销此活动</a>
					<?php  } ?>
				<?php  } ?>
			</div>
		</div>
