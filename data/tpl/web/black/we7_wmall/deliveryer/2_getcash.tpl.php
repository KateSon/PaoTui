			...
			<table class="table table-hover">
				...
				<?php  if(is_array($records)) { foreach($records as $record) { ?>
					<tr>
						...
						<td align="right">
							<?php  if($record['status'] == 2) { ?>
								<a href="<?php  echo iurl('deliveryer/getcash/transfers', array('id' => $record['id']));?>" data-confirm="确定微信打款吗?" class="btn btn-primary btn-sm js-post">微信打款</a>
								<a href="<?php  echo iurl('deliveryer/getcash/status', array('id' => $record['id'], 'status' => 1));?>" data-confirm="确定变更提现状态吗?" class="btn btn-default btn-sm js-post">设为已处理</a>
								<a href="<?php  echo iurl('deliveryer/getcash/cancel', array('id' => $record['id']));?>" class="btn btn-danger btn-sm js-modal">撤销</a>
							<?php  } ?>
						</td>
					</tr>
				<?php  } } ?>
