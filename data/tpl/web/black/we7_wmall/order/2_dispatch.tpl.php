					...
					<div class="form-group">
						<select name="deliveryer_id" id="select-deliveryer" class="form-control">
							...
							<?php  if(is_array($deliveryer_alls)) { foreach($deliveryer_alls as $alls) { ?>
								<option value="<?php  echo $alls['deliveryer_id'];?>" <?php  if($alls['deliveryer_id'] == $deliveryer_id) { ?>selected<?php  } ?>><?php  echo $alls['deliveryer']['title'];?></option>
							<?php  } } ?>
						</select>
					</div>
				</form>
...
<script type="text/javascript" src="//webapi.amap.com/maps?v=1.3&key=550a3bf0cb6d96c3b43d330fb7d86950&plugin=AMap.Driving,AMap.Geocoder,AMap.ToolBar"></script>
<script>
		...
		$.post("<?php  echo iurl('order/dispatch/map')?>", {sid: sid, deliveryer_id: deliveryer_id }, function(data){
			...
			$.each(dispatch.deliveryers, function(k, v) {
				var content = '<div class="map-mark '+v.css+'" id="map-mark-'+ v.id+'" data-id="'+ v.id+'" data-css="'+v.css+'">'+
						...
						'		<img class="location-dot" src="<?php echo WE7_WMALL_URL;?>static/img/location_dot.png" alt=""/>'+
						'	</div>';
