...
<div class="page" id="page-manage-goods">
	...
	<div class="content">
		<div class="buttons-tab">
			<a href="<?php  echo imurl('manage/goods/category/list', array('status' => -1));?>" class="button <?php  if($status == -1) { ?>active<?php  } ?>">所有分组</a>
			<a href="<?php  echo imurl('manage/goods/category/list', array('status' => 1));?>" class="button <?php  if($status == 1) { ?>active<?php  } ?>">上架中</a>
			<a href="<?php  echo imurl('manage/goods/category/list', array('status' => 0));?>" class="button <?php  if($status == 0) { ?>active<?php  } ?>">已下架</a>
		</div>
		<?php  if(empty($lists)) { ?>
			<div class="no-data">
				<div class="bg"></div>
			...
			<?php  if(is_array($lists)) { foreach($lists as $list) { ?>
				<div class="group-item border-1px-tb">
					...
					<div class="group-item-operate">
						...
						<a href="javascript:;" class="goods-category-edit border-1px-r" data-id="<?php  echo $list['id'];?>" data-title="<?php  echo $list['title'];?>" data-displayorder="<?php  echo $list['displayorder'];?>">编辑</a>
						<?php  if($list['status'] == 1) { ?>
