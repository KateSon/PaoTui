...
<div class="page getcash">
	...
	<div class="content">
		<div class="takeout-title border-1px-tb">账户可用余额：<span>¥<?php  echo $store['account']['amount'];?></span></div>
		<ul class="takeout-list border-1px-tb">
			<li>
				...
				<div class="takeout-item-right">
					...
					<p class="takeout-rule">最低提现金额为<?php  echo $store['account']['fee_limit'];?>元</p>
					<p class="takeout-rule">提现费率为<?php  echo $store['account']['fee_rate'];?>%,最低收取<?php  echo $store['account']['fee_min'];?>元<?php  if($store['account']['fee_max'] > 0) { ?>,最高收取<?php  echo $store['account']['fee_max'];?>元<?php  } ?></p>
					<?php  if($store['account']['amount'] < $store['account']['fee_limit']) { ?>
						<a href="#" class="button button-big button-fill button-success disabled">不足<?php  echo $store['account']['fee_limit'];?>元</a>
					<?php  } else { ?>
