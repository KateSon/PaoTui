...
<div class="page page-current-log">
	...
	<div class="content infinite-scroll js-infinite" data-href="<?php  echo imurl('manage/finance/current', array('trade_type' => $trade_type))?>" data-distance="50" data-min="<?php  echo $min;?>" data-container=".record-container" data-tpl="tpl-inout">
		<div class="buttons-tab">
			<a href="<?php  echo imurl('manage/finance/current', array('trade_type' => '0'))?>" class="button <?php  if($trade_type == 0) { ?>active<?php  } ?>">全部</a>
			<a href="<?php  echo imurl('manage/finance/current', array('trade_type' => '1'))?>" class="button <?php  if($trade_type == 1) { ?>active<?php  } ?>">外卖店内入账</a>
			<a href="<?php  echo imurl('manage/finance/current', array('trade_type' => '4'))?>" class="button <?php  if($trade_type == 4) { ?>active<?php  } ?>">买单入账</a>
			<a href="<?php  echo imurl('manage/finance/current', array('trade_type' => '2'))?>" class="button <?php  if($trade_type == 2) { ?>active<?php  } ?>">提现</a>
			<a href="<?php  echo imurl('manage/finance/current', array('trade_type' => '3'))?>" class="button <?php  if($trade_type == 3) { ?>active<?php  } ?>">其他</a>
		</div>
	...
	<div class="content trade-details border-1px-tb">
		...
		<?php  if($current['trade_type'] == 2) { ?>
			...
			<div class="row no-gutter border-1px-b">
				...
				<div class="col-80 text-right color-black"><?php  echo $getcash_log['account']['realname'];?></div>
			</div>
			<div class="row no-gutter border-1px-b">
