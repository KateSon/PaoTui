		...
		<div class="order-list">
			<?php  if(empty($orders)) { ?>
				<div class="common-no-con">
					...
					<p>您还没有订单，赶紧点一份！</p>
					<div class="btn">
						<a href="<?php  echo imurl('wmall/home/index');?>">现在点一份</a>
					</div>
				</div>
			<?php  } else { ?>
					...
					<div class="order-container">
						...
						<div class="order-btn table border-1px-t">
							...
							<?php  if($order['is_refund'] == 1) { ?>
								<a href="<?php  echo imurl('wmall/order/index/detail', array('id' => $order['id']));?>" class="table-cell border-1px-l">查看退款</a>
							<?php  } ?>
						</div>
					</div>
				<?php  } } ?>
	...
	<div class="order-container">
		...
		<div class="order-btn table border-1px-t">
			<{# if(!d[i].is_pay && d[i].status != 5 && d[i].status != 6){ }>
				<a href="<?php  echo imurl('system/paycenter/pay', array('order_type' => 'takeout', 'type' => 1));?>&id=<{d[i].id}>" class="table-cell border-1px-r">立即支付</a>
			<{# } }>
			<{# if(d[i].status == 1){ }>
				<a href="<?php  echo imurl('wmall/order/index/cancel');?>&id=<{d[i].id}>" class="table-cell js-post" data-confirm="确定取消该订单吗">取消订单</a>
				<{# if(d[i].is_pay == 1){ }>
					<a href="<?php  echo imurl('wmall/order/index/remind');?>&id=<{d[i].id}>" class="table-cell border-1px-l js-post">催单</a>
				<{# } }>
			...
			<{# } else if(d[i].status == 5) { }>
				<a href="<?php  echo imurl('wmall/store/goods', array('f' => '1'));?>&id=<{d[i].id}>&sid=<{d[i].sid}>" class="table-cell border-1px-r" data-id="<?php  echo $order['id'];?>">再来一单</a>
				<{# if(d[i].is_comment == 1){ }>
			...
			<{# } }>
			<{# if(d[i].is_refund == 1){ }>
				<a href="<?php  echo imurl('wmall/order/order', array('op' => 'detail'));?>&id=<{d[i].id}>" class="table-cell border-1px-l">查看退款</a>
			<{# } }>
		</div>
	</div>
	<{# } }>
