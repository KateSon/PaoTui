...
$sid = intval($_GPC['sid']);
file_put_contents('member.txt',json_encode($_W['member']));

$user=pdo_get('tiny_wmall_order', array('uid' =>$_W['member']['uid']),array('username','sex','mobile'));
file_put_contents('user.txt',json_encode($user));
...
if($ta == 'submit') {
	...
	if($_GPC['order_type'] == 1 || $_GPC['order_type'] == 4) {
