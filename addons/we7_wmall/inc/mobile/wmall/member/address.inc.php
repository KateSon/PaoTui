...
$routes = array('order' => imurl('wmall/order/create/index', array('sid' => $_GPC['sid'], 'r' => 1,'pay_id'=>$_GPC['pay_id'], 'recordid' => $_GPC['recordid'], 'redPacket_id' => $_GPC['redPacket_id'])) . '&address_id=');
$redirect_url = $routes[$redirect_type];

if ($ta == 'location') {
	$pay_id=intval($_GPC['pay_id']);
...
if ($ta == 'selcity') {
	...
	$location=$_GPC['location'].'&area='.$_GPC['area'];
	//var_dump($location);
	imessage(error(0, $location), '', 'ajax');
}
