...
$payment = array_keys(get_plugin_config('advertise.payment'));
if(empty($payment)) {
	...
	if($pay_type == 'wechat') {
		if(is_weixin()) {
			...
			if(in_array($config_wechat['type'], array('borrow', 'borrow_partner'))) {
				...
				$url = imurl("manage/paycenter/cash/{$pay_type}", array(), true);
			...
			$url = imurl('manage/paycenter/wxpay', array('auth' => $auth, 'ps' => $sl));
			header("Location:{$url}");
			exit();
		} else {
			...
			if(!empty($log) && $log['status'] == '0') {
				...
				if(!is_error($site)) {
					...
					$method = 'payResult';
						...
						echo '<iframe style="display:none;" src="'.imurl('manage/paycenter/cash/credit', array('notify' => 'yes', 'params' => $_GPC['params']), true).'"></iframe>';
						$site->$method($ret);
					}
				}
			}
