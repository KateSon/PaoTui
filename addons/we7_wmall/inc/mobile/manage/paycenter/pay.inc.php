...
$tables = array(
	'advertise' => 'tiny_wmall_advertise_trade',
);
...
$routers = array(
	'advertise' => array(
		'title' => "{$order['title']}-{$record['order_sn']}",
		'url_pay' => imurl('manage/paycenter/pay', array('id' => $order['id'], 'order_type' => 'advertise'), true),
		//'url_detail
		'url_detail' => imurl("manage/advertise/{$order['type']}", array('id' => $order['id']), true),
	),
);
...
if(empty($log)) {
	$log = array(
		'uniacid' => $_W['uniacid'],
		'acid' => $_W['acid'],
		'openid' => $params['openid'] ? $params['openid'] : $params['user'],
...
if($order['final_fee'] == 0) {
	...
	header('location:' . imurl("manage/paycenter/cash/credit" , array('params' => $params)));
	die;
}

$payment = array('wechat', 'credit');
if(empty($payment)) {
	imessage('没有有效的支付方式, 请联系网站管理员.', '', 'error');
}
$pay_type = !empty($_GPC['pay_type']) ? trim($_GPC['pay_type']) : $order['pay_type'];
if($pay_type && !$_GPC['type'] && in_array($pay_type, $payment)) {
	$params = base64_encode(json_encode($params));
	header('location:' . imurl("manage/paycenter/cash/{$pay_type}" , array('params' => $params)));
	die;
}
