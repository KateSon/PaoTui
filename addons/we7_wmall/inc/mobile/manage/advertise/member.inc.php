...
$slideMember= get_advertise_info('slideMember');
if($ta == 'index') {
	$_W['page']['title'] = '会员中心幻灯片';
	include itemplate('advertise/member');
}
if($ta == 'submit') {
	if($_W['isajax']) {
		if(!$slideMember['leave']) {
		...
		$finalfee = $slideMember['prices'][$day]['fee'];
		...
		pdo_insert('tiny_wmall_advertise_trade', $memberData);
		$id = pdo_insertid();
