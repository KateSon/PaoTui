...
if ($op == 'index') {
	...
	if ($_W['isajax']) {
		$chart = array(
			'fields' => array('total_success_order', 'order_eleme', 'order_meituan', 'order_wxapp', 'order_wap', 'order_h5app', 'order_plateformCreate'),
			'titles' => array('有效订单量', '饿了么订单', '美团订单', '小程序订单', 'H5订单', '顾客APP订单', '后台创建订单')
			);
		$i = $starttime;
		...
		foreach ($channels as $key => $value) {
			...
			$order = 'order_' . $key;
			$one_order = pdo_fetchall('select stat_day, count(*) as ' . $order . ' from ' . tablename('tiny_wmall_order') . ($condition . ' and order_channel = :order_channel group by stat_day'), $params);
