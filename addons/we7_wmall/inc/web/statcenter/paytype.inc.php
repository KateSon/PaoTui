...
if ($op == 'index') {
	...
	if ($_W['isajax']) {
		$chart = array(
			'fields' => array('total_success_order', 'order_delivery', 'order_credit', 'order_wechat', 'order_alipay'),
			'titles' => array('有效订单量', '货到付款订单', '余额支付订单', '微信支付订单', '支付宝支付订单')
			);
		$i = $starttime;
		...
		foreach ($paytypes as $key => $value) {
			...
			$order = 'order_' . $key;
			$one_order = pdo_fetchall('select stat_day, count(*) as ' . $order . ' from ' . tablename('tiny_wmall_order') . ($condition . ' and pay_type = :pay_type group by stat_day'), $params);
	...
	$basic = array('stat_day' => 0, 'total_success_order' => 0, 'order_delivery' => 0, 'order_credit' => 0, 'order_wechat' => 0, 'order_alipay' => 0);
	$i = $endtime;
	...
	while ($starttime <= $i) {
		...
		$data = array_merge($basic, $records_all['total_success_order'][$i], $records_all['order_delivery'][$i], $records_all['order_credit'][$i], $records_all['order_wechat'][$i], $records_all['order_alipay'][$i]);
