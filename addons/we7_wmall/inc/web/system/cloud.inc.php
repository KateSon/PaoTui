...
if ($op == 'auth') {
	...
	if ($_W['ispost']) {
		...
		if (is_error($status)) {
			...
			$data = array('cloud_id' => $status['message']['cloud_id'], 'code' => $status['message']['code'], 'code_status' => 1);
...
if ($op == 'upgrade') {
	...
	if (!empty($_W['setting']['copyright']['status'])) {
		$url = url('system/site');
		imessage('更新时不能关闭站点。现在去开启: <a href="' . $url . '" target="_blank">' . $url . '</a>', '', 'error');
	}
