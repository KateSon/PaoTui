...
if (!is_weixin()) {
	imessage(base64_decode('6K+35Zyo5b6u5L+h5Lit6K6/6Zeu6K+l6ZO+5o6l'), '', base64_decode('aW5mbw=='));
}
...
if ($ta == 'message') {
	...
	if (empty($peerpay)) {
		...
		pdo_insert(base64_decode('dGlueV93bWFsbF9vcmRlcl9wZWVycGF5'), $peerpay);
		$peerpay['id'] = pdo_insertid();
	}

	pdo_update(base64_decode('dGlueV93bWFsbF9vcmRlcg=='), array(base64_decode('cGF5X3R5cGU=') => base64_decode('cGVlcnBheQ==')), array(base64_decode('dW5pYWNpZA==') => $_W['uniacid'], 'id' => $peerpay['orderid']));
...
if ($ta == 'payment') {
	...
	if ($_W['ispost']) {
		...
		if (empty($is_exist)) {
			...
			pdo_update(base64_decode('dGlueV93bWFsbF9vcmRlcl9wZWVycGF5X3BheWluZm8='), $insert, array('id' => $is_exist['id']));
			$id = $is_exist['id'];
		}

		imessage(error(0, imurl(base64_decode('c3lzdGVtL3BheWNlbnRlci9wYXk='), array(base64_decode('b3JkZXJfdHlwZQ==') => base64_decode('cGVlcnBheQ=='), base64_decode('aWQ=') => $id))), '', 'ajax');
	}
}
...
include itemplate(base64_decode('cGF5Y2VudGVyL3BlZXJwYXk='));
