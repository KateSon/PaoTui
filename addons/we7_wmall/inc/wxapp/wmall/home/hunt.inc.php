...
if ($ta == 'index') {
	...
	$result = array('hotStores' => store_fetchall_by_condition('hot'), 'recommendStores' => store_fetchall_by_condition('recommend'), 'searchHistorys' => $member['search_data']);
...
if ($ta == 'search') {
	...
	if (!empty($goods)) {
		...
		$stores = pdo_fetchall('select id,title,logo,content,business_hours,delivery_fee_mode,delivery_price,delivery_areas,send_price,delivery_time,delivery_mode,forward_mode,forward_url,score,label,sailed,send_price from ' . tablename('tiny_wmall_store') . (' where uniacid = :uniacid and agentid = :agentid and status = 1 and id in (' . $sids_str . ')'), array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid']), 'id');
	}

	$search_stores = pdo_fetchall('select id,title,logo,content,business_hours,delivery_fee_mode,delivery_price,delivery_areas,send_price,delivery_time,delivery_mode,forward_mode,forward_url,score,label,sailed,send_price from ' . tablename('tiny_wmall_store') . (' where uniacid = :uniacid and agentid = :agentid and status = 1 and id not in (' . $sids_str . ') and title like :key'), array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':key' => '%' . $key . '%'));
	...
	$result = array('stores' => $stores, 'recommendStores' => $recommend_stores);
	imessage(error(0, $result), '', 'ajax');
}
