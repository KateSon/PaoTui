...
class subscribe extends Meituan
{
	...
	public function parse()
	{
		...
		if (1 || in_array($type, array(10, 12))) {
			...
			$store = pdo_get('tiny_wmall_store', array('meituanShopid' => $shopid), array('id', 'title', 'location_y', 'location_x', 'data', 'meituan_status'));
		...
		if ($type == 'order') {
			...
			if (!empty($account['fee_meituan'])) {
				if ($account['fee_meituan'] == 1) {
					$platform_serve_fee = floatval($account['fee_meituan']['fee']);
					$plateform_serve = array('fee_type' => 1, 'fee_rate' => 0, 'fee' => $platform_serve_fee, 'note' => '每单固定' . $platform_serve_fee . '元', 'meituan' => $meituan);
				}
				else {
					$platform_serve_fee = round($order['meituan_store_final_fee'] * ($account['fee_meituan']['fee_rate'] / 100), 2);
					$plateform_serve = array('fee_type' => 2, 'fee_rate' => $account['fee_meituan']['fee_rate'], 'fee' => $platform_serve_fee, 'note' => '(美团平台最终入账 ￥' . $order['meituan_store_final_fee'] . ') x ' . $account['fee_meituan']['fee_rate'] . '%', 'meituan' => $meituan);
				}
			}
	...
	public function start()
	{
		...
		if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
			if ($op == 'storemap') {
				file_put_contents(MODULE_ROOT . '/meituan.txt', var_export($_POST, 1));
				$this->saveAppAuthToken($_POST);
				return NULL;
			}

			if ($op == 'releasebinding') {
				file_put_contents(MODULE_ROOT . '/meituan_releasebinding.txt', var_export($_POST, 1));
