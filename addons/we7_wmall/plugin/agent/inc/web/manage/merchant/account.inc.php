...
if ($op == 'index') {
	...
	$condition = ' as a left join ' . tablename('tiny_wmall_store_account') . ' as b on a.id = b.sid where a.uniacid = :uniacid and a.agentid = :agentid';
...
if ($op == 'set') {
	...
	if ($_W['ispost']) {
		...
		$data = array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'sid' => $sid, 'fee_takeout' => iserializer($fee_takeout), 'fee_selfDelivery' => iserializer($fee_selfDelivery), 'fee_instore' => iserializer($fee_instore), 'fee_paybill' => iserializer($fee_paybill), 'fee_limit' => intval($_GPC['get_cash_fee_limit']), 'fee_rate' => trim($_GPC['get_cash_fee_rate']) ? trim($_GPC['get_cash_fee_rate']) : 0, 'fee_min' => intval($_GPC['get_cash_fee_min']), 'fee_max' => intval($_GPC['get_cash_fee_max']));

		if ($_GPC['fee_eleme']['fee_type'] == 1) {
			$data['fee_eleme'] = iserializer(array('fee_type' => 1, 'fee_rate' => floatval($_GPC['fee_eleme']['fee_rate'])));
		}
		else {
		...
		if ($_GPC['fee_meituan']['fee_type'] == 1) {
			$data['fee_meituan'] = iserializer(array('fee_type' => 1, 'fee_rate' => floatval($_GPC['fee_meituan']['fee_rate'])));
		}
		else {
		...
		if ($update['delivery_fee_mode'] == 1) {
			...
			$update['delivery_price'] = iserializer(array('start_fee' => trim($_GPC['start_fee']), 'start_km' => trim($_GPC['start_km']), 'pre_km_fee' => trim($_GPC['pre_km_fee'])));
		}
		else {
