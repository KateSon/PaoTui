...
if ($op == 'list') {
	...
	if (!empty($navs)) {
		$ids = implode(',', array_keys($navs));
		$nums = pdo_fetchall('SELECT count(*) AS num,cid FROM ' . tablename('tiny_wmall_store') . (' WHERE uniacid = :uniacid AND agentid = :agentid and  cid IN (' . $ids . ') GROUP BY cid'), array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid']), 'cid');
	}
...
if ($op == 'post') {
	...
	if ($_W['ispost']) {
		...
		$data = array('uniacid' => $_W['uniacid'], 'title' => $title, 'agentid' => $_W['agentid'], 'thumb' => trim($_GPC['thumb']), 'link' => trim($_GPC['link']), 'displayorder' => intval($_GPC['displayorder']), 'nav_status' => intval($_GPC['nav_status']), 'slide_status' => intval($_GPC['slide_status']), 'nav' => iserializer($nav), 'slide' => iserializer($slide), 'wxapp_link' => trim($_GPC['wxapp_link']));
