...
if ($op == 'map') {
	...
	if ($_W['ispost']) {
		$condition = ' where uniacid = :uniacid and agentid = :agentid and status = 3 and delivery_type = 2';
		...
		$condition = ' where a.uniacid = :uniacid and a.agentid = :agentid and a.sid = 0 and a.work_status = 1';
		$params = array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid']);
		...
		if (!empty($deliveryers)) {
			foreach ($deliveryers as &$row) {
				$row['finish'] = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_order') . ' where uniacid = :uniacid and agentid = :agentid and deliveryer_id = :deliveryer_id and stat_day = :stat_day and delivery_status = 5', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':deliveryer_id' => $row['id'], ':stat_day' => date('Ymd')));
				$row['wait_pickup'] = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_order') . ' where uniacid = :uniacid and agentid = :agentid and deliveryer_id = :deliveryer_id and stat_day = :stat_day and delivery_status = 7', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':deliveryer_id' => $row['id'], ':stat_day' => date('Ymd')));
				$row['wait_delivery'] = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_order') . ' where uniacid = :uniacid and agentid = :agentid and deliveryer_id = :deliveryer_id and stat_day = :stat_day and delivery_status = 4', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':deliveryer_id' => $row['id'], ':stat_day' => date('Ymd')));
				$row['work_status'] = 1;
...
if ($op == 'deliveryer') {
	if ($_W['ispost']) {
		...
		$params = array(':uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], ':deliveryer_id' => $id, ':stat_day' => date('Ymd'));
		...
		$deliveryer['wait_pickup'] = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_order') . ' where uniacid = :uniacid and agentid = :agentid and deliveryer_id = :deliveryer_id and stat_day = :stat_day and delivery_status = 7', $params);
