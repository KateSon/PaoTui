...
if ($op == 'post') {
	...
	if ($_W['ispost']) {
		...
		$data = array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'mobile' => $mobile, 'title' => trim($_GPC['title']), 'openid' => $openid, 'nickname' => trim($_GPC['wechat']['nickname']), 'avatar' => trim($_GPC['wechat']['avatar']), 'sex' => trim($_GPC['sex']), 'age' => intval($_GPC['age']));
