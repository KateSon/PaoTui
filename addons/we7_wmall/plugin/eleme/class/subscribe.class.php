...
class subscribe extends Eleme
{
	...
	public function parse()
	{
		...
		if (1 || in_array($type, array(10, 12))) {
			...
			$store = pdo_get('tiny_wmall_store', array('elemeShopid' => $shopid), array('id', 'title', 'location_y', 'location_x', 'data', 'eleme_status'));
		...
		if ($type == 10) {
			...
			$order = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'sid' => $store['id'], 'uid' => 0, 'order_type' => 1, 'elemeOrderId' => $message['orderId'], 'ordersn' => $message['orderId'], 'address' => $message['address'], 'addtime' => strtotime($message['activeAt']), 'paytime' => strtotime($message['activeAt']), 'note' => $message['description'], 'invoice' => $message['invoice'], 'mobile' => $message['phoneList'][0], 'total_fee' => $message['originalPrice'], 'final_fee' => $message['totalPrice'], 'store_final_fee' => 0, 'eleme_store_final_fee' => $message['income'], 'username' => substr($message['consignee'], 0, strpos($message['consignee'], '(')), 'sex' => substr($message['consignee'], strpos($message['consignee'], '(') + 1, 6), 'location_x' => $deliveryGeo[1], 'location_y' => $deliveryGeo[0], 'order_plateform' => 'eleme', 'order_channel' => 'eleme', 'delivery_type' => 0, 'delivery_fee' => $message['deliverFee'], 'delivery_time' => !empty($message['deliverTime']) ? $message['deliverTime'] : '立即配送', 'is_pay' => 1, 'pay_type' => $message['onlinePaid'] ? 'eleme' : 'delivery', 'serial_sn' => $message['daySn'], 'plateform_serve_rate' => $message['serviceRate'], 'plateform_serve_fee' => $message['serviceFee'], 'box_price' => $message['packageFee'], 'plateform_discount_fee' => $message['elemePart'], 'store_discount_fee' => $message['shopPart'], 'discount_fee' => $message['activityTotal'], 'plateform_delivery_fee' => 0, 'status' => $this->orderStatusTransform($message['status']), 'stat_year' => date('Y', $message['activeAt']), 'stat_month' => date('Ym', $message['activeAt']), 'stat_day' => date('Ymd', $message['activeAt']));
			...
			if (!empty($account['fee_eleme'])) {
				if ($account['fee_eleme'] == 1) {
					$platform_serve_fee = floatval($account['fee_eleme']['fee']);
					$plateform_serve = array('fee_type' => 1, 'fee_rate' => 0, 'fee' => $platform_serve_fee, 'note' => '每单固定' . $platform_serve_fee . '元', 'eleme' => $eleme);
				}
				else {
					$platform_serve_fee = round($message['income'] * ($account['fee_eleme']['fee_rate'] / 100), 2);
					$plateform_serve = array('fee_type' => 2, 'fee_rate' => $account['fee_eleme']['fee_rate'], 'fee' => $platform_serve_fee, 'note' => '(饿了么平台最终入账 ￥' . $message['income'] . ') x ' . $account['fee_eleme']['fee_rate'] . '%', 'eleme' => $eleme);
				}
			}
