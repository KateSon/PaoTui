...
class Eleme
{
	...
	public function __construct($sid = 0)
	{
		...
		$this->app = array(
			'sid'    => $sid,
			'store'  => array('eleme_status' => $store['eleme_status'], 'eleme' => $store['data']['eleme'], 'shopid' => $store['elemeShopId']),
			'key'    => $this->config['key'],
	...
	public function getOauthCodeUrl($callback, $state = 'xyz')
	{
		...
		return $this->api_url . 'authorize?response_type=code&client_id=' . $this->app['key'] . '&redirect_uri=' . $callback . '&state=' . $state . '&scope=all';
	}
	...
	public function getAccessTokenByCode($code, $callback)
	{
		...
		$response = ihttp_request($this->api_url . 'token', http_build_query($body), $extra);
	...
	public function httpPost($action, $params)
	{
		...
		$protocol = array(
			...
			'action' => $action,
			'token'  => $access_token,
			'metas'  => array('app_key' => $this->app['key'], 'timestamp' => TIMESTAMP),
			'params' => $params
			);
		...
		$response = ihttp_request($this->api_url . 'api/v1/', json_encode($protocol), array('Content-Type' => 'application/json'));
		...
		if (!empty($result['error'])) {
			if ($result['error']['code'] == 'UNAUTHORIZED') {
				return error(-1, $result['error']['code'] . ': ' . $result['error']['message'] . ',请重新去进行饿了么授权');
			}
